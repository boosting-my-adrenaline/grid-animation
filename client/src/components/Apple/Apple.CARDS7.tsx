...
import { LandingCardRectSmall } from './Apple.card.rect.small'
import { LandingCardRectSmall2 } from './Apple.card.rect.small2'

interface IProps {
  sm: boolean
}
export const LandingCARDS7: React.FC<IProps> = ({ sm }) => {
      ...
      >
        ...
        >
          <li className={`h-full ${snap} `} ref={startRef}>
            <LandingCardRectSmall2 sm={false} option={0} />
          </li>

          {[1, 2, 3].map((el) => (
            <li key={el} className={`h-full ${snap} `}>
              <LandingCardRectSmall2 sm={false} option={el} />
            </li>
          ))}

          <li className={`h-full ${snap} `} ref={endRef}>
            <LandingCardRectSmall2 sm={false} option={4} />
          </li>
        </ul>
        {width > 1000 && (
          <LandingArrow
